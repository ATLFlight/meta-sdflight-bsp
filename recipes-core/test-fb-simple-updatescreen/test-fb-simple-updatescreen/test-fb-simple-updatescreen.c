...
void draw_grid(int w, int h, void* _loc, int offset) {
    int i, j;
    int v;
    ...
    for (j = 0; j < h/2; j++) {
        for (i = 0; i < w/2; i++)
            if (vi.bits_per_pixel == 16)
                loc[i + j*(stride)] = c16( offset );
            else
                loc32[i + j*(stride)] = c32( offset );
        for (; i < w; i++)
            if (vi.bits_per_pixel == 16)
                loc[i + j*(stride)] = c16( offset + 1 );
            else
                loc32[i + j*(stride)] = c32( offset + 1 );
    }

    for (; j < h; j++) {
        for (i = 0; i < w/2; i++)
            if (vi.bits_per_pixel == 16)
                loc[i + j*(stride)] = c16( offset + 2 );
            else
                loc32[i + j*(stride)] = c32( offset + 2 );
        for (; i < w; i++)
            if (vi.bits_per_pixel == 16)
                loc[i + j*(stride)] = c16( offset + 3 );
            else
                loc32[i + j*(stride)] = c32( offset + 3 );
    }
...
int main(int argc, char **argv) {
  ...
  while( 1 )
  {
     id++;
     id %= 2;
     draw_grid(w, h, gr_fbs[ id ].data, offset);
     set_active_framebuffer( id );
     printf( "offset %d\n", offset );
     if ( bpp != -1 ) usleep( bpp );
     offset++;
